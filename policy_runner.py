...
def main():
    ...
    cache = Cache(policy_class(), Storage(args.cache_size))
    ...
    with tqdm.tqdm(total=request_count, desc="Simulating caching policy") as pbar:
        for req in traffic_reader.read_traffic():
            hit = cache.access(req.object_id, req.timestamp)
    ...
    stats = state.to_dict()
    logger.info(f"Stats: hits={stats['hit_count']}, misses={stats['miss_count']}, accesses={stats['access_count']}, size={stats['current_size']}/{stats['capacity']}")
