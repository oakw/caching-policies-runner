...
with open (os.path.join(BASE_DIR, 'config.json'), 'r') as file:
    ...
    for i in tqdm.tqdm(range(len(config)), desc="Running configurations"):
        run = config[i]
...
for key, results in grouped_results.items():
    ...
    avg_stats = {}
    for stat in ['hits', 'misses', 'accesses', 'current_size']:
...
md = "| Policy | Model | Cache Size | Hits | Misses | Accesses | Current Size | User Time | System Time | Elapsed Time | Max RSS | Exit Code |"
md += "\n|--------|-------|------------|------|--------|----------|--------------|-----------|-------------|--------------|---------|-----------|"
for result in averaged_results:
    md += f"\n| {result['run']['policy']} | {result['run']['model']} | {result['run']['cache_size']} | {result['stats']['hits']:.1f} | {result['stats']['misses']:.1f} | {result['stats']['accesses']:.1f} | {result['stats']['current_size']:.1f} | {result['timing']['user_time']:.3f} | {result['timing']['system_time']:.3f} | {result['timing']['elapsed_time']:.3f} | {result['timing']['max_rss']:.1f} | {result['timing']['exit_code']} |"
